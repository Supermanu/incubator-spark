...
class PrimitivesSuite extends FunSuite with LocalSparkContext {

  test("udpateVerticesUsingLocalEdges") {
    withSpark { sc =>
      ...
      val newGraph = graph.updateVerticesUsingLocalEdges[VertexId](EdgeDirection.In,
        (vid, vdata, edges) => {
          if (edges.isEmpty) { -1 }
  ...
  test("aggregateNeighborValuesNbrPredicate") {
    withSpark { sc =>
      ...
      val resultGraph = getGraphForAggregateNeighborValuesTest(sc).aggregateNeighborValues[Int](
        EdgeDirection.Out, (nid, nvalue) => (nid <= 2), (vid, vvalue) => true,
        (vid, vvalue) => vvalue,
        (a, b) => a + b, (vid, vvalue, optAggregatedValue) => {
          (vvalue + optAggregatedValue.getOrElse(0))
        })
  ...
  test("aggregateNeighborValuesUpdateFunction") {
    withSpark { sc =>
      ...
      // to 1, 0 and 1�should remain unchanged. and 2 should update it to 0 because the
      // we set the default aggregated value to 0.
      val resultGraph = getGraphForAggregateNeighborValuesTest(sc).aggregateNeighborValues[Int](
        EdgeDirection.Out, (nid, nvalue) => true, (vid, vvalue) => vid > 1, (vid, vvalue) => vvalue,
        (a, b) => a + b, (vid, vvalue, optAggregatedValue) =>
          Math.min(vvalue, optAggregatedValue.getOrElse(0)))
      assert(5 == resultGraph.numVertices)
      assert(6 == resultGraph.numEdges)
      resultGraph.vertices.collect.foreach {
        case (vid, value) =>
  ...
  test("propagateAndAggregateDirectionIn") {
    withSpark { sc =>
      // We are propagating towards in neighbors. Therefore, when the aggregation is the min, 3 and
      // 4 should update their values to 1 and 2, respectively, and others should remain unchanged.
  ...
  test("propagateAndAggregateDirectionOut") {
    withSpark { sc =>
      // We are propagating towards out neighbors. Therefore, when the aggregation is the max, 1 and
      // 2 should update their values to 3 and 4, respectively, and others should remain unchanged.
      val resultGraph = getGraphForPropagateAndAggregateTest(sc).propagateAndAggregate[Int](
  ...
  test("propagateAndAggregateStartVertexPredicate") {
    withSpark { sc =>
      // We are propagating towards out neighbors, starting from vertex 3. Therefore, when the
      // aggregation is the max, 1 should update its value to 3 and others should remain unchanged.
      val resultGraph = getGraphForPropagateAndAggregateTest(sc).propagateAndAggregate[Int](
        EdgeDirection.Out,
  ...
  test("propagateAndAggregateDirectionPropagateWithEdgeDistances") {
    withSpark { sc =>
      ...
      // When we propagate the distances by including the edge values, we expect the result
      // distances to be: 1 has distance 1, 2 has distance 2, 3 has distance 3, 4 has distance 4
      // and 5 has distance 1
      val edges = sc.parallelize((1 to 1).flatMap(x => {
        Vector(Edge(0, 1, 1), Edge(0, 4, 10), Edge(1, 2, 1), Edge(2, 3, 1), Edge(3, 4, 1),
          Edge(0, 5, 1))
      }))
  ...
  test("updateAnotherVertexUsingSelfAllPointingToDifferentVertices") {
    withSpark { sc =>
      ...
      val resultGraph = getGraphForUpdateVertexUsingOneOtherVertex(sc)
        .updateAnotherVertexUsingSelf[Long](
          (vid, vvals) => true,
          (vid, vvals) => vvals._2,
          (vid, vvals) => vid,
  ...
  test("updateAnotherVertexUsingSelfVPred") {
    withSpark { sc =>
      ...
      // the update is now done only by vertices with id >= 2. Therefore, now vertices 1 and 2's 
      // first fields remain as -2.
      val resultGraph = getGraphForUpdateVertexUsingOneOtherVertex(sc)
        .updateAnotherVertexUsingSelf[Long](
          (vid, vvals) => vid >= 2,
          (vid, vvals) => vvals._2,
  ...
  test("updateAnotherVertexUsingSelfAggregateRelevantValuesF") {
    withSpark { sc =>
      // 6 vertices. The edges are irrelevant for this primitive but here are the 8 edges for
      // reference: 0 -> 1, 0 -> 2, 1 -> 0, 2 -> 0, 2 -> 1, 3 -> 4, 4 -> 5, 5 -> 0
      val edges = sc.parallelize((1 to 1).flatMap(x => {
        Vector(Edge(0, 1, -1), Edge(0, 2, -1), Edge(1, 0, -1), Edge(2, 0, -1), Edge(2, 1, -1),
          Edge(3, 4, -1), Edge(4, 5, -1), Edge(5, 0, -1))
      }))
      // Vertex values have two fields, the first is set to -2 for all, and the second are the
      // pointers to other vertices: 0 to 2, 1 to 2, 2 to 0, 3 to 1, 4 to 2, 5 to 1
      val vertices = sc.parallelize((0 to 5).map(vid => {
        var pointedVertex = -1
      ...
      // We also test that the aggregation function is called only when there are at least two
      // vertices updating a vertex.
  ...
  test("updateSelfUsingAnotherVertexAllPointingToDifferentVertices") {
    withSpark { sc =>
      ...
      val resultGraph = getGraphForUpdateVertexUsingOneOtherVertex(sc)
        .updateSelfUsingAnotherVertex[Long](
          (vid, vvals) => true,
          (vid, vvals) => vvals._2,
          (vid, vvals) => vid,
          (vid, vvals, relevantValue) => (relevantValue, vvals._2))
  ...
  test("updateSelfUsingAnotherVertexVPred") {
    withSpark { sc =>
      ...
      // the update is now done only by vertices with id >= 2. Therefore, now vertices 0 and 1's
      // first fields remain as -2.
      val resultGraph = getGraphForUpdateVertexUsingOneOtherVertex(sc)
        .updateSelfUsingAnotherVertex[Long](
          (vid, vvals) => vid >= 2,
  ...
  test("aggregateGlobalValueWithLocalEdges") {
    withSpark { sc =>
      // We aggregate the product of the number of edges each vertex has, except singletons
      // multiplies by 1. Result: 3*2*2 = 12
      val resultValue = getGraphForAggregateGlobalValueTest(sc)
        .aggregateGlobalValueWithLocalEdges[Int](
  ...
  test("formSuperverticesSimple") {
    withSpark { sc =>
      ...
      val edges = sc.parallelize((1 to 1).flatMap(x => {
        Vector(Edge(1, 0, -1), Edge(1, 2, -1), Edge(3, 2, -1), Edge(3, 4, -1), Edge(5, 4, -1),
          Edge(5, 6, -1), Edge(9, 6, -1))
      }))
  ...
  test("formSuperverticesAggregateVertices") {
    withSpark { sc =>
      ...
      // But vertices now have two fields. The first one is the supervertex identifier field as in
      // test formSuperverticesSimple. The second field, which we will aggregate by taking its min,
      // contains vid* 100, e.g., 0 contains 0, 1 contains 100, 2 contains 200, etc.
      val vertices = sc.parallelize((0 to 9).map(vid => {
        val vertexId: VertexId = vid
        var firstField = -1L
  ...
  private def getGraphForUpdateVertexUsingOneOtherVertex(sc: SparkContext):
    Graph[(Long, Long), Int] = {
    // 5 vertices. The edges are irrelevant for this primitive but here are the 7 edges for
    // reference: 0 -> 1, 0 -> 2, 1 -> 0, 2 -> 0, 2 -> 1, 3->4, 4->3
    ...
    // Vertex values have two fields, the first is set to -2 for all, and the second are the
    // pointers to other vertices: 0 points to 1, 1 points to 2, 2 points to 4, 3 points to 0,
    // 4 points to 3
  ...
  private def getGraphForAggregateNeighborValuesTest(sc: SparkContext): Graph[Int, Int] = {
    ...
    val edges = sc.parallelize((1 to 1).flatMap(x => {
      Vector(Edge(1, 0, -1), Edge(1, 2, -1), Edge(1, 3, -1), Edge(0, 1, -1), Edge(3, 1, -1),
        Edge(4, 1, -1))
    }))
    // Vertex i have value i
    val vertices = sc.parallelize((0 to 4).map(vid => (vid: VertexId, vid)))
