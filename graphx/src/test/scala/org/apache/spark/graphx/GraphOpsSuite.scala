...
//  }
    ...
    withSpark { sc =>
      val edges = sc.parallelize((1 to 1).flatMap(x => {
        Vector(Edge(1, 2, 120), Edge(1, 3, 130), Edge(3, 2, 320), Edge(1, 4, 140), Edge(2, 4, 240))
      }))
      // Same vertices as in test formSuperverticesSimple
      val vertices = sc.parallelize((0 to 9).map(vid => {
        val vertexId: VertexId = vid
        var value: (Array[VertexId], List[(VertexId, Array[VertexId])], Int) = (Array.empty, List.empty, -1)
        (vertexId, value)
      }))
      var graph = Graph(vertices, edges).cache()
      // Take the max of edge values
      graph = graph.updateVerticesUsingLocalEdges(EdgeDirection.Either, 
        (vid, vdata, edges) => {
           val localEdges = edges.map(edge => {edge.otherVertexId(vid)}); (localEdges, vdata._2, -1)})
      graph = graph.aggregateNeighborValues[List[(VertexId, Array[VertexId])]](EdgeDirection.Either,
        (vId, vdata) => true, (vId, vdata) => true, 
        (vId, vdata) => List((vId, vdata._1)), (val1, val2) => val1:::val2,
        (vId, vdata, optAggrValue) => (vdata._1, optAggrValue.getOrElse(List.empty), -1))
      graph = graph.updateVertices((vId, vdata) => true,
        (vid, vdata) => {
          val nbrsSet = vdata._1.toSet
          var numTriangles = 0;
          for ((nbrId, nbrArrayList) <- vdata._2) {
           for (otherNbrId <- nbrArrayList) {
             if (nbrsSet.contains(otherNbrId)) numTriangles += 1
           } 
          }
          (vdata._1, vdata._2, numTriangles)
        })
        println(graph.vertices.collect().deep.mkString("\n"))
    }
  }
